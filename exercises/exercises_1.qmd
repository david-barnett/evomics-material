...
These exercises will cover some of the core concepts in microbiome data analysis, using example data.
You can follow along by writing code in your own R script (or in an R Markdown notebook, or in a Quarto doc).

-   [**Part 1**](#intro-diversity): phyloseq objects for microbiome data; stacked bar charts for visualizing microbiome compositions; and ecosystem diversity indices.

-   [**Part 2**](#dissimilarity-ordination-da): ecological dissimilarity measures; PCoA; PERMANOVA; PCA; and Differential Abundance testing.
...
```{r}
data_directory <- here::here("data") # set to correct AMI path to data folder
...
-   We'll primarily use these two datasets in these exercises
...
-   These methods are also appropriate for microbiome data from other environments, not just the gut microbiota! (microViz has been used by groups analysing microbiomes associated with Antarctic sponges, and Kiwi birds, amongst other things.)

------------------------------------------------------------------------

## 1: Intro & Diversity {#intro-diversity}
...
ggsave(plot = plotsWNV, filename = "WNVplots.png", height = 12, width = 8, dpi = 200)
```
...
- The mice exposed to Ampicillin (alone or with Metronidazole) appear to have higher diversity at this day 7 sample!
...
You can apply more complex statistical tests if you like, e.g. adjusting for covariates with linear regression, using `lm()`
